...
window.CondoHunting=(()=>{
  ...
  function render(){const root=document.getElementById('condo-root');if(!root)return;window.CondoMap?.destroy();const legacy=document.getElementById('hm-legacy');legacy.hidden=mode!=='explore';root.hidden=mode==='explore';document.querySelectorAll('[data-condo-mode]').forEach(b=>b.setAttribute('aria-pressed',b.dataset.condoMode===mode));if(mode==='explore'){hmLegacyRender();return;}
    ...
    if(comparisonIndex!==null&&data[comparisonIndex]){root.innerHTML=unitComparison(data[comparisonIndex]);return;}
  ...
  const preferenceRank=o=>o['Unit preference']==='First choice'?0:o['Unit preference']==='Alternative'?2:1;
  const preferenceLabel=o=>o['Unit preference']?`<span class="ch-preference ${o['Unit preference']==='First choice'?'is-first':''}">${e(o['Unit preference'])}</span>`:'';
  function unitPhoto(o){return url(o['Photo URL'])?`<a class="ch-unit-photo" href="${e(url(o.id))}" target="_blank" rel="noopener noreferrer" aria-label="View listing photos for ${e(o.name)}"><img src="${e(url(o['Photo URL']))}" alt="${e(o.name)} — listing photo" loading="lazy" onerror="this.parentElement.hidden=true"><span>View listing photos ↗</span></a>`:'';}
  function relatedUnits(o){return sheetItems().filter(x=>building(x)===building(o)&&!archived(x)).sort((a,b)=>preferenceRank(a)-preferenceRank(b)||Number(a['Rent RM/month'])-Number(b['Rent RM/month']));}
  function unitComparison(o){
    const units=relatedUnits(o),preferred=units.find(x=>x['Unit preference']==='First choice'),price=Number(preferred?.['Rent RM/month']);
    return `<section id="ch-units" class="ch-units"><button class="btn" onclick="CondoHunting.closeCompare()">← Back to reviews</button><div class="ch-units-heading"><span class="ch-area">Your saved units</span><h2>${e(o['Building name']||o.name.split(' · ')[0])}</h2><p>Compare the exact units, with your first choice first. Listing details still need confirmation with each agent.</p></div><nav class="ch-unit-summary" aria-label="Jump to a saved unit">${units.map((x,i)=>`<a href="#ch-unit-option-${i}" onclick="event.preventDefault();document.getElementById('ch-unit-option-${i}').scrollIntoView({block:'start'});document.getElementById('ch-unit-option-${i}').focus({preventScroll:true})"><span>${e(x['Unit preference']||'Saved unit')}</span><strong>RM ${Number(x['Rent RM/month']).toLocaleString()}</strong><small>${e((x.Agent||x.name).split(' · ')[0])} ↓</small></a>`).join('')}</nav><div class="ch-unit-grid">${units.map((x,i)=>{
      const rent=Number(x['Rent RM/month']),delta=price&&rent?rent-price:null,index=data.findIndex(d=>d.id===x.id),st=status(x);
      return `<article id="ch-unit-option-${i}" tabindex="-1" class="ch-unit ${x===preferred?'is-first':''}">${unitPhoto(x)}<div class="ch-unit-body">${preferenceLabel(x)}<h3>${e(x.name)}</h3><div class="ch-price">RM ${rent.toLocaleString()}<small> / month</small></div><p class="ch-delta">${x===preferred?'Your preferred unit':delta!==null?(delta===0?'Same rent as first choice':`RM ${Math.abs(delta).toLocaleString()} ${delta>0?'more':'less'} / month than first choice`):'No preferred unit selected'}</p><dl><div><dt>Space</dt><dd>${e(x['Size sqft']||'Unknown')} sqft · ${e(x.Bedrooms||'?')} beds · ${e(x.Bathrooms||'?')} baths</dd></div><div><dt>Tower & view</dt><dd>${e(x['Balcony / floor / view']||'Not confirmed')}</dd></div><div><dt>Furnishing</dt><dd>${e(x.Furnishing||'Not confirmed')}</dd></div><div><dt>Agent</dt><dd>${e(x.Agent||'Not confirmed')}</dd></div><div><dt>Review status</dt><dd>${e(st)}</dd></div><div><dt>Gym</dt><dd>${e(x['Gym summary']||x['Gym equipment']||'Not confirmed')}</dd></div></dl><div class="ch-links">${link(x.id,'Listing & photos ↗')}${link(x['Agent contact']||x['Primary agent WhatsApp'],'Contact agent ↗')}${index>=0?`<button class="btn" onclick="CondoHunting.details(${index})">Full research</button>`:''}</div></div></article>`;
    }).join('')}</div></section>`;
  }
  function card({o,index,v,st}){
    const peers=relatedUnits(o);
    const review=o['Review notes']||'',personal=DB.get('condoReviewNotes',{})[o.id]||'';
    return `<article class="ch-card ch-review" id="ch-card-${index}">${unitPhoto(o)}<div class="ch-heading"><div><span class="ch-area">${e(o.Neighbourhood)} · ${e(o['Build year filter']||'Year unconfirmed')}</span><h3>${e(o.name)}</h3>${preferenceLabel(o)}<span class="ch-status ${st==='❤️ Both shortlist'?'shortlisted':''}">${e(st)}</span></div></div><div class="ch-body"><div class="ch-price">RM ${Number(o['Rent RM/month']).toLocaleString()}<small> / month</small></div><p class="ch-spec">${e(o['Size sqft'])} sqft · ${e(o.Bedrooms)} beds · ${e(o.Bathrooms)} baths</p><div class="ch-links ch-quick">${link(o['Agent contact']||o['Primary agent WhatsApp'],'Contact agent ↗')}${peers.length>1?`<button class="btn" onclick="CondoHunting.compare(${index})">Compare ${peers.length} units</button>`:''}</div><p class="ch-furnishing">${e(brief(o.Furnishing||'Furnishing unconfirmed',105))}</p>${url(o['Condo YouTube tour'])?`<div class="ch-tour">${link(o['Condo YouTube tour'],'▶ '+(o['Condo tour label']||'Watch condo tour'))}</div>`:''}<div class="ch-facts"><div><h4>🚗 CEVA → home</h4><p>${commute(o)}</p><small>Evening drive · traffic varies</small></div><div><h4>🏊 Pool</h4><p>${pool(o)}</p></div><div><h4>🏋️ Gym</h4><p>• ${e(brief(o['Gym summary']||o['Gym equipment']||'Equipment not verified.',120))}</p></div><div><h4>🚶 Around you</h4><p>• ${e(brief(o['Area summary']||o['Walkability evidence']||'Walkability not verified.',120))}</p></div><div><h4>🛗 Lifts</h4><p>• ${e(brief(o['Lift summary']||'Lift count and units per floor unconfirmed.',120))}</p></div></div>${review?`<div class="ch-decision"><h4>Review decision</h4><p>• ${e(brief(review,140))}</p>${o['Review date']?`<small>${e(o['Review date'])}</small>`:''}</div>`:''}<div class="ch-votes">${[['F','Fares'],['C','Charlotte']].map(([k,n])=>`<div><label class="ch-vlabel" for="ch-${index}-${k}">${n}</label><select id="ch-${index}-${k}" ${st.startsWith('Archived')?'disabled':''} onchange="CondoHunting.vote(${index},'${k}',this.value)">${verdicts.map(x=>`<option ${v[k]===x?'selected':''}>${e(x)}</option>`).join('')}</select></div>`).join('')}</div>${st.startsWith('Archived')?'<p class="ch-sync">Archived condo: other units here will not be proposed again. Its review history is kept here.</p>':''}<label class="ch-note">Add a review note<textarea rows="2" onchange="CondoHunting.note(${index},this.value)" placeholder="What works, what rules it out…">${e(personal)}</textarea></label><div class="ch-links">${link(o.id,'Listing ↗')}${link(o['Area YouTube tour'],'Area tour ↗')}<button class="btn" onclick="CondoHunting.details(${index})">Full research</button></div></div></article>`;
  }
  function property(l){
  ...
  return {relatedUnits,unitComparison,compare:index=>{comparisonIndex=index;detail=null;discovery=null;mode='comparison';render();document.getElementById('ch-units')?.scrollIntoView({block:'start'});},closeCompare:()=>{comparisonIndex=null;render();},render,refresh,connect,vote,note,acceptLink,parse,commuteRoute,eligible,sourceId,mergeSource,status,archived,blockedDiscovery,areaDecision,fullyFurnished,property,research,discoveryDetails,back,details:index=>{detailBack=mode;detail=index;discovery=null;render();},filter:k=>{comparisonIndex=null;filter=k==='all'?'active':k;detail=null;render();},mode:k=>{comparisonIndex=null;mode=k;detail=null;discovery=null;render();}};
})();
